...
[dropZZZPrevFaas]
drop table zzz_migrate_prevfaas 
...
[createZZZPrevFaas]
create table zzz_migrate_prevfaas(
	objid varchar(50)
	primary key(objid)
)
...
[insertZZZPrevFaasData]
insert into zzz_migrate_prevfaas 
	(objid)
select objid from etracs25_migrate_prevfaas
from faaslist fl 
where not exists(select * from etracs25_migrate_faas where objid = fl.objid)
...
SELECT * FROM faaslist fl
where not exists(SELECT * FROM etracs25_migrate_prevfaas where objid = fl.objid) 
AND not exists(SELECT * FROM etracs25_migrate_prevfaas_log where objid = fl.objid) 
